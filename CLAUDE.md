...
| `headroom/aws/policy_documents.py` | [`contracts/policy-model.md`](spec/contracts/policy-model.md), every `checks/rcps/*.md` | Every adapter that reads a statement: `headroom/aws/ecr.py`, `kms.py`, `s3.py`, `secretsmanager.py`, `sqs.py`, `iam/roles.py`. A change to how a statement is read is a change to all six. `read_statement_principals` is the one rule all six read a statement's principals by - it composes `_read_principal`, which stays the element reader and which nothing outside this module calls. `tests/test_aws_policy_documents.py` pins both: six copies of the walk once disagreed four ways, and an adapter reaching `_read_principal` directly would read a condition-confined wildcard as unconfined. `_condition_clauses` is the one parse of a `Condition` block, shared by its three readers in this module, and the same test file pins that no reader of the block exists outside it. |
