...
### Step 1.1: Create Enums Module
**Time:** 30 minutes
**Risk:** Low
**Files to create:** `headroom/enums.py`
...
**Files to update:**
- `headroom/checks/base.py` - Import and use CheckCategory
- `headroom/checks/registry.py` - Import and use CheckType
- `headroom/parse_results.py` - Import and use PlacementLevel
- `headroom/terraform/generate_scps.py` - Import and use PlacementLevel
- `headroom/terraform/generate_rcps.py` - Import and use PlacementLevel
- All check implementations - Use CheckCategory instead of strings

**Test:** Run `tox` to ensure no regressions

---

### Step 1.2: Extract Account Identifier Formatting
**Time:** 20 minutes
**Risk:** Low
**Files to modify:**

**Create new function in `headroom/utils.py` (new file):**
```python
...
**Update these files:**
1. `headroom/analysis.py` - Line 309, 190
2. `headroom/checks/base.py` - Line 190
3. `headroom/write_results.py` - Line 145

**Search pattern:** `f"{.*}_{.*account_id}"` to find all occurrences

**Test:** Run `tox`

---

### Step 1.3: Add Constants for Magic Numbers
**Time:** 15 minutes
**Risk:** Low
**Files to modify:**

**Add to `headroom/constants.py`:**
```python
...
**Update:**
- `headroom/terraform/generate_scps.py` - Line 76
- `headroom/parse_results.py` - Multiple locations checking `violations == 0`
...
### Step 2.1: Create ResultFilePathResolver Class
**Time:** 2 hours
**Risk:** Medium
**Files to modify:** `headroom/write_results.py`
...
**Refactor existing functions to use resolver:**
```python
def write_check_results(
    ...
    results_dir = resolver.get_check_directory()
    os.makedirs(results_dir, exist_ok=True)

    output_file = resolver.get_file_path()
...
def get_results_dir(check_name: str, results_base_dir: str) -> str:
    ...
    resolver = ResultFilePathResolver(
        check_name=check_name,
        account_name="",  # Not needed for directory lookup
        account_id="",  # Not needed for directory lookup
        results_base_dir=results_base_dir
    )
...
**Test:**
- Run existing tests in `tests/test_write_results.py`
- Add new tests for ResultFilePathResolver class
- Run `tox`
