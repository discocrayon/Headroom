...
def _ecr_paginator(
    template_paginator: MagicMock,
    repository_paginator: MagicMock,
    operation_name: str,
) -> MagicMock:
    """
    Hand back the paginator for one ECR operation.

    The analyzer reads two paginated operations from one client, so a single
    `get_paginator.return_value` cannot serve both. Bound with
    `functools.partial` as the mock client's `get_paginator.side_effect`; an
    operation neither name covers raises KeyError rather than answering.
    """
    return {
        "describe_repository_creation_templates": template_paginator,
        "describe_repositories": repository_paginator,
    }[operation_name]
...
class TestRegistryPolicy:
    ...
    def test_repository_and_registry_are_reported_separately(self) -> None:
        """Repository and registry are separate resources, so each gets its own row."""
        results = self._analyze(
            registry_policy=self._policy(
                {"AWS": f"arn:aws:iam::{self.THIRD_PARTY}:root"}
            ),
...
class TestCreationTemplates:
    """
    Repository creation templates, whose policy ECR attaches to every
    repository it creates from them.

    A repository policy governs a repository that exists. A template's
    `repositoryPolicy` governs repositories that do not exist yet, so a third
    party named there is denied the day such a repository is created unless
    the RCP already allowlists it.
    """
    ORG_ACCOUNT = "111111111111"
    THIRD_PARTY = "999999999999"
    ...
    def _mock_ecr(
        ...
        """
        Build a session over a single region, with no registry policy unless one is given.

        Args:
            template_pages: Pages describe_repository_creation_templates
                returns, each a dict with a `repositoryCreationTemplates` list
            template_error: AWS error code the templates paginator should
                raise instead of yielding pages, which takes precedence over
                template_pages
            repositories: Repositories the region holds - empty by default, so
                that any result can only have come from a template
            repository_policy: Policy returned for those repositories, or None
                for repositories that carry no policy
            registry_response: Raw get_registry_policy() response to return
                verbatim, or None for a registry that carries no policy

        Returns:
            The mock session and the mock ECR client it hands out
        """
        mock_session = MagicMock()
        mock_ec2_client = MagicMock()
        mock_ecr_client = MagicMock()
        ...
        if template_error is None:
            template_paginator.paginate.return_value = template_pages
        else:
            failure: Any = {"Error": {"Code": template_error}}
            template_paginator.paginate.side_effect = ClientError(
                failure, "DescribeRepositoryCreationTemplates"
            )
        repository_paginator = MagicMock()
        repository_paginator.paginate.return_value = [{"repositories": repositories or []}]
        mock_ecr_client.get_paginator.side_effect = partial(
            _ecr_paginator, template_paginator, repository_paginator
        )
    ...
    def _analyze(
        ...
        """
        Run the analyzer over a single region, with no registry policy unless one is given.

        Args:
            template_pages: Pages describe_repository_creation_templates
                returns, each a dict with a `repositoryCreationTemplates` list
            repositories: Repositories the region holds, empty by default
            repository_policy: Policy returned for those repositories, or None
                for repositories that carry no policy
            registry_response: Raw get_registry_policy() response to return
                verbatim, or None for a registry that carries no policy
        ...
        Returns:
            The analyzer's results for the region
        """
        mock_session, _ = TestCreationTemplates._mock_ecr(
            template_pages,
        ...
        return analyze_ecr_policies(
            mock_session, {TestCreationTemplates.ORG_ACCOUNT}, ORG_ID
        )

    def test_third_party_in_template_policy_is_found(self) -> None:
        """A third party named only in a template still reaches the allowlist."""
        results = self._analyze([{
            "repositoryCreationTemplates": [{
                "prefix": "team-a",
                "repositoryPolicy": json.dumps(self._policy(
                    {"AWS": f"arn:aws:iam::{self.THIRD_PARTY}:root"}
                )),
            }]
        }])
        ...
        assert len(results) == 1
        assert results[0].scope == "creation_template"
        assert results[0].template_prefix == "team-a"
        assert results[0].repository_name is None
        assert results[0].repository_arn is None
        assert results[0].region == "us-east-1"
        assert results[0].third_party_account_ids == {self.THIRD_PARTY}

    def test_wildcard_template_policy_is_flagged(self) -> None:
        """A wildcard here reaches everyone, in every repository the template creates."""
        results = self._analyze([{
            "repositoryCreationTemplates": [{
                "prefix": "team-a",
                "repositoryPolicy": json.dumps(self._policy({"AWS": "*"})),
            }]
        }])
    ...
    def test_template_without_policy_reports_nothing(self) -> None:
        """
        `repositoryPolicy` is optional, and its absence means the template
        attaches no policy - a fact about the template, not a failed read.
        """
        results = self._analyze([{
            "repositoryCreationTemplates": [{"prefix": "team-a"}]
        }])

        assert results == []

    def test_template_policy_naming_only_org_accounts_reports_nothing(self) -> None:
        """An in-org grant is not third-party access."""
        results = self._analyze([{
            "repositoryCreationTemplates": [{
                "prefix": "team-a",
    ...
    def test_templates_on_every_page_are_read(self) -> None:
        """A second page of templates is read like the first."""
        third_party = self._policy({"AWS": f"arn:aws:iam::{self.THIRD_PARTY}:root"})
        results = self._analyze([
            {"repositoryCreationTemplates": [
                {"prefix": "team-a", "repositoryPolicy": json.dumps(third_party)}
            ]},
            {"repositoryCreationTemplates": [
                {"prefix": "team-b", "repositoryPolicy": json.dumps(third_party)}
            ]},
        ])

        assert len(results) == 2
        assert {result.template_prefix for result in results} == {"team-a", "team-b"}

    def test_templates_are_read_through_their_own_paginator(self) -> None:
        """
        The read names describe_repository_creation_templates, so a client
        answering describe_repositories alone cannot satisfy it.
        """
        mock_session, mock_ecr_client = self._mock_ecr(
            [{"repositoryCreationTemplates": []}]
        )

        analyze_ecr_policies(mock_session, {self.ORG_ACCOUNT}, ORG_ID)

        assert (
            call("describe_repository_creation_templates")
            in mock_ecr_client.get_paginator.call_args_list
        )

    def test_template_listing_error_propagates(self) -> None:
        """
        Failing to list templates aborts the run rather than reporting nothing.

        A scan role lacking ecr:DescribeRepositoryCreationTemplates is answered
        with AccessDeniedException, and a region whose templates could not be
        read is not a region with no templates (INV-01).
        """
        mock_session, _ = self._mock_ecr([], template_error="AccessDeniedException")
    ...
    def test_template_and_repository_are_reported_separately(self) -> None:
        """
        The surfaces are separate resources, so each gets its own row, and
        the template comes first: it reaches repositories the repository
        loop will never see.
        """
        results = self._analyze(
            [{"repositoryCreationTemplates": [{
        ...
        assert [result.scope for result in results] == ["creation_template", "repository"]
        assert results[0].template_prefix == "team-a"
        assert results[0].third_party_account_ids == {self.THIRD_PARTY}
        assert results[1].repository_name == "test-repo"
        assert results[1].third_party_account_ids == {"888888888888"}

    def test_federated_principal_in_a_template_blocks_the_template(self) -> None:
        """
        A Federated principal carries no account ID the allowlist can hold.

        The RCP restricts on aws:PrincipalAccount, which cannot express a
        Federated principal, so the template is recorded as a violation rather
        than aborting the run - and every repository ECR creates from it
        inherits the same grant.
        """
        results = self._analyze([{
            "repositoryCreationTemplates": [{
                "prefix": "team-a",
    ...
    def test_guarded_service_principal_in_a_template_is_recorded(self) -> None:
        """
        A template pinning a third-party source records it.

        The template's analysis copies the field in its own construction, and
        a template whose only grant is a guarded service principal reaches
        the results on that field alone, so the copy is pinned here rather
        than through the repository path.
        """
        results = self._analyze([{
            "repositoryCreationTemplates": [{
                "prefix": "team-a",
                "repositoryPolicy": json.dumps({
                    ...
                    "Statement": [{
                        ...
                        "Action": "ecr:BatchGetImage",
                        "Condition": {
                            "StringEquals": {"aws:SourceAccount": self.THIRD_PARTY}
                        },
                    }],
                }),
            }]
        ...
        source = results[0].service_principal_sources[0]
        assert source.service_principal == "sns.amazonaws.com"
        assert source.source_account_ids == [self.THIRD_PARTY]
        assert results[0].third_party_account_ids == set()

    def test_registry_template_and_repository_are_reported_in_read_order(self) -> None:
        """
        A region reports its registry policy, then its templates, then its repositories.

        The specification states the per-region read order as registry, then
        templates, then repositories, and the existing pair tests pin only the
        template-before-repository link. This is a characterization test over
        the order the analyzer already reads in, so it passes at once.
        """
        third_party = self._policy({"AWS": f"arn:aws:iam::{self.THIRD_PARTY}:root"})
        results = self._analyze(
            [{"repositoryCreationTemplates": [{
                "prefix": "team-a",
                "repositoryPolicy": json.dumps(third_party),
            }]}],
            repositories=[
                {
                    "repositoryName": "test-repo",
            ...
            repository_policy=third_party,
            registry_response={"policyText": json.dumps(third_party)},
        )

        assert [result.scope for result in results] == ["registry", "creation_template", "repository"]
