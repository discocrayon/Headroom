...
def test_render_scp_terraform_no_recommendations(tmp_path: Path) -> None:
    ...
    assert render_scp_terraform([], org, tmp_path, coverage_for("scps")) == {}
...
def test_render_scp_terraform_warn_missing_account(tmp_path: Path) -> None:
    ...
    with pytest.raises(RuntimeError, match="Account \\(999999999999\\) not found in organization hierarchy"):
        render_scp_terraform([rec], org, tmp_path, coverage_for("scps"))
...
def test_build_scp_terraform_module_output_format() -> None:
    ...
    result = _build_scp_terraform_module(
        ...
        target_id=org.root_id,
        placed=placed_targets(recs, org),
        coverage=coverage_for("scps"),
        flipped={},
        flipped_comment={},
    )
...
def test_build_scp_terraform_module_rejects_a_none_level_recommendation() -> None:
    ...
    with pytest.raises(RuntimeError, match=r"scps_root.*deny_ec2_imds_v1.*'none'"):
        _build_scp_terraform_module(
            ...
            organization_hierarchy=org,
            target_id=org.root_id,
            placed=placed_targets([none_rec], org),
            coverage=coverage_for("scps"),
            flipped={},
            flipped_comment={},
        )
...
def test_render_account_scp_terraform_names_the_file_for_the_account() -> None:
    ...
    filepath, content = _render_account_scp_terraform(
        "333333333333", [rec], org, output_path, placed_targets([rec], org), coverage_for("scps"), {}, {}
    )
...
def test_render_account_scp_terraform_rejects_a_folded_name_collision() -> None:
    ...
    with pytest.raises(RuntimeError, match="prod_us"):
        _render_account_scp_terraform(
            "111111111111", [rec], org, Path("/nonexistent"), placed_targets([rec], org), coverage_for("scps"), {}, {}
        )
...
def test_render_account_scp_terraform_raises_error_for_missing_account() -> None:
    ...
    with pytest.raises(RuntimeError, match="Account \\(999999999999\\) not found in organization hierarchy"):
        _render_account_scp_terraform(
            "999999999999", [rec], org, Path("/nonexistent"), placed_targets([rec], org), coverage_for("scps"), {}, {}
        )
...
def test_render_ou_scp_terraform_names_the_file_for_the_ou() -> None:
    ...
    filepath, content = _render_ou_scp_terraform(
        "ou-test", [rec], org, output_path, placed_targets([rec], org), coverage_for("scps"), {}, {}
    )
...
def test_render_ou_scp_terraform_raises_error_for_missing_ou() -> None:
    ...
    with pytest.raises(RuntimeError, match="OU ou-missing not found in organization hierarchy"):
        _render_ou_scp_terraform(
            "ou-missing", [rec], org, Path("/nonexistent"), placed_targets([rec], org), coverage_for("scps"), {}, {}
        )
...
def test_render_root_scp_terraform_renders_root_scps_tf() -> None:
    ...
    filepath, content = _render_root_scp_terraform(
        [rec], org, output_path, placed_targets([rec], org), coverage_for("scps"), {}, {}
    )
...
def test_render_root_scp_terraform_multiple_checks() -> None:
    ...
    _, content = _render_root_scp_terraform(
        recs, org, Path("/nonexistent"), placed_targets(recs, org), coverage_for("scps"), {}, {}
    )
...
def test_render_root_scp_terraform_includes_saml_guardrail() -> None:
    ...
    _, content = _render_root_scp_terraform(
        [rec], org, Path("/nonexistent"), placed_targets([rec], org), coverage_for("scps"), {}, {}
    )
...
def test_plan_omits_root_when_nothing_is_placed_there(tmp_path: Path) -> None:
    ...
    plan = render_scp_terraform([rec], org, tmp_path, coverage_for("scps"))
...
def test_an_account_named_root_does_not_take_the_root_policy_file(tmp_path: Path) -> None:
    ...
    with pytest.raises(RuntimeError, match="root_scps.tf"):
        render_scp_terraform(recommendations, org, tmp_path, coverage_for("scps"))
...
def test_render_scp_terraform_returns_the_rendered_content_by_destination(
    ...
    rendered = render_scp_terraform([make_rec()], org, tmp_path, coverage_for("scps"))
...
def test_a_failed_render_raises_before_any_file_is_produced(tmp_path: Path) -> None:
    ...
    with pytest.raises(RuntimeError, match="not found in organization hierarchy"):
        render_scp_terraform([good, doomed], org, tmp_path, coverage_for("scps"))
...
def test_render_scp_terraform_raises_when_an_account_collides_with_root(tmp_path: Path) -> None:
    ...
    with pytest.raises(RuntimeError, match="root_scps.tf"):
        render_scp_terraform([account_rec, root_rec], org, tmp_path, coverage_for("scps"))
...
def test_an_ou_file_says_the_root_already_enforces_the_check(tmp_path: Path) -> None:
    ...
    The OU's file exists because one check placed there; every other check
    renders false, and a reader cannot otherwise tell which of those the
    root already covers.

    `deny_ec2_imds_v1` needs an account outside the OU that blocked it, or
    the OU placement could not have happened: the traversal only descends
    past the root when something under the root is unsafe. One account
    cannot be both the OU's safe placement and the root's reason to descend.
    """
    org = make_nested_org()
    ...
    plan = render_scp_terraform(recommendations, org, tmp_path, coverage)
    content = plan[tmp_path / "production_ou_scps.tf"]

    assert "  # Enforced at the organization root\n  deny_ec2_ami_owner = false" in content
    assert "  deny_ec2_imds_v1 = true" in content
    assert "  # No results for this check - not evidence of safety\n  deny_ec2_public_ip = false" in content
...
def _render_every_scp_check(observed_nothing: Sequence[str] = ()) -> str:
    ...
    return _build_scp_terraform_module(
        ...
        organization_hierarchy=org,
        target_id=org.root_id,
        placed=placed_targets(recommendations, org),
        coverage=coverage_for("scps"),
        flipped={},
        flipped_comment={},
    )
...
def test_build_scp_terraform_module_iam_user_creation_empty_allowlist_stays_off() -> None:
    ...
    assert "iam_allowed_users" not in result
    assert (
        "  # deny_iam_user_creation stays off here: no IAM user in the accounts this\n"
        "  # module covers, so the allowlist would be empty - and NotResource: [] is\n"
        "  # rejected as a malformed policy.\n"
        "  deny_iam_user_creation = false\n"
    ) in result
...
def test_a_root_placement_an_empty_allowlist_flipped_off_claims_nothing_below(
    tmp_path: Path
) -> None:
    """
    A root file rendering false cannot make an OU file say it is enforced.

    `deny_ec2_ami_owner` placed at the root with nothing to allowlist is off
    at the root (INV-06), so "Enforced at the organization root" in the OU
    below it is the INV-01 failure the comments exist to remove. The OU
    carries the check's own sentence instead, which stays true there: an
    empty union over the root's accounts is empty over the OU's subset.

    The OU's account has to be one `deny_ec2_ami_owner` actually judged. The
    sentence describes what the covered accounts held, so below a flipped
    placement it renders only where something was scanned - a target with no
    results of its own says that instead (INV-01).
    """
    org = make_nested_org()
    recommendations = [
        make_rec(check_name="deny_ec2_ami_owner", level="root", allowlist_values=[]),
        make_rec(
            check_name="deny_ec2_imds_v1",
            level="ou",
            target_ou_id="ou-1111-11111111",
    ...
    ou_content = plan[tmp_path / "production_ou_scps.tf"]
    assert "Enforced at the organization root\n  deny_ec2_ami_owner" not in ou_content
    assert (
        "  # deny_ec2_ami_owner stays off here: no instance in the accounts this\n"
        in ou_content
    )
    assert "  deny_ec2_ami_owner = false\n" in ou_content


def test_a_root_placement_the_allowlist_did_populate_is_still_enforced_below(
    tmp_path: Path
) -> None:
    """
    Only the flipped placement loses shape 1; a live one keeps it.

    The two are one boolean apart in the same file, so a fix that dropped
    every allowlist check out of `placed` would pass the test above and
    silently replace this comment with a blocked or no-results claim.
    """
    org = make_nested_org()
    recommendations = [
        make_rec(
    ...
    plan = render_scp_terraform(recommendations, org, tmp_path, coverage_for("scps"))

    assert (
        "  # Enforced at the organization root\n  deny_ec2_ami_owner = false\n"
        in plan[tmp_path / "production_ou_scps.tf"]
    )


def test_one_check_live_at_one_ou_and_flipped_at_its_sibling(tmp_path: Path) -> None:
    """
    Two placements of one check, one live and one an empty allowlist turned off.

    This is why the enabled and flipped targets are two maps rather than one
    set of check names: keyed by name, whichever placement was seen last
    would answer for both subtrees, and one of the two answers would be
    false. Under the live OU the account still reads `Enforced at OU
    production`; under the flipped one it reads the check's own sentence.

    The coverage is what makes the wrong answer available: with the flipped
    ancestor unrecognised, fort-knox falls through to shape 3 and reads
    `Blocked by this account's violations` - of an account this run analyzed
    and found clean.
    """
    org = OrganizationHierarchy(
        root_id="r-1111",
    ...
    assert "  deny_ec2_ami_owner = true\n" in plan[tmp_path / "production_ou_scps.tf"]
    assert own_comment in plan[tmp_path / "sandbox_ou_scps.tf"]
    assert (
        "  # Enforced at OU production\n  deny_ec2_ami_owner = false\n"
        in plan[tmp_path / "acme_co_scps.tf"]
    )
    assert own_comment in plan[tmp_path / "fort_knox_scps.tf"]


def test_a_none_recommendation_for_an_allowlist_check_still_renders_the_run(tmp_path: Path) -> None:
    """
    No account is safe for an allowlist check: the run still renders, and says why.

    Placement materializes a `none` recommendation for the check and leaves
    its allowlist at None, having unioned nothing over no accounts. The split
    that files placements as live or flipped must drop it before asking what
    its allowlist renders, or one unresolvable AMI owner in every account
    aborts Terraform generation for the whole organization.
    """
    org = make_nested_org()
    recommendations = [
        make_rec(check_name="deny_ec2_imds_v1", level="root"),
        SCPPlacementRecommendations(
            check_name="deny_ec2_ami_owner",
            recommended_level="none",
            target_ou_id=None,
            affected_accounts=[],
            compliance_percentage=0.0,
            reasoning="No accounts have zero violations",
        ),
    ]
    coverage = coverage_for("scps", **{
        "deny_ec2_ami_owner": CheckCoverage(
    ...
    assert (
        "  # Blocked by the only analyzed account (acme-co)\n  deny_ec2_ami_owner = false\n"
        in plan[tmp_path / "root_scps.tf"]
    )
