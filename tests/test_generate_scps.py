...
def test_generate_scp_terraform_raises_when_an_account_collides_with_root(tmp_path: Path) -> None:
    """
    An account named `root` renders to root_scps.tf, the root file's own name.

    The SCP generator writes accounts before root, so the account claims the
    ...
    with pytest.raises(RuntimeError, match="root_scps.tf"):
        generate_scp_terraform([account_rec, root_rec], org, str(tmp_path))
