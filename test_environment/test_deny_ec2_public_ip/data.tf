...
# Get the default VPC for each account to launch instances in
data "aws_vpc" "default_acme_co" {
  provider = aws.acme_co
  default  = true
}

data "aws_vpc" "default_fort_knox" {
  provider = aws.fort_knox
  default  = true
}

data "aws_vpc" "default_shared_foo_bar" {
  provider = aws.shared_foo_bar
  default  = true
}

# Get subnets from default VPCs
data "aws_subnets" "default_acme_co" {
  provider = aws.acme_co
  filter {
    name   = "vpc-id"
    values = [data.aws_vpc.default_acme_co.id]
  }
}

data "aws_subnets" "default_fort_knox" {
  provider = aws.fort_knox
  filter {
    name   = "vpc-id"
    values = [data.aws_vpc.default_fort_knox.id]
  }
}

data "aws_subnets" "default_shared_foo_bar" {
  provider = aws.shared_foo_bar
  filter {
    name   = "vpc-id"
    values = [data.aws_vpc.default_shared_foo_bar.id]
  }
}
