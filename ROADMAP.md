...
- Condition-aware RCP analysis, the remainder: a wildcard guarded only by `ArnLike aws:SourceArn` still blocks its account, and a grant narrowed by `s3:prefix`, `aws:SourceVpce`, `kms:ViaService`, or a date condition still contributes its account at full width. Bounding a wildcard by `aws:PrincipalOrgID`, `aws:PrincipalOrgPaths`, `aws:PrincipalAccount`, `aws:PrincipalArn`, or `kms:CallerAccount` already ships. Reading `aws:SourceArn` as a bound is settled against - see [`spec/contracts/policy-model.md`](spec/contracts/policy-model.md) - so what is open is whether another signal, such as CloudTrail evidence of the actual caller, closes the gap without un-blocking on the key
