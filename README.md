...
- **Output**: Per-provider records with ARN, derived name, creation/expiration timestamps, and violation reasons
  - **Why safe to deny everyone?** The `AWSServiceRoleForSSO` service-linked role provisions the official provider during IAM Identity Center onboarding and is not affected by SCPs, so blocking `iam:CreateSAMLProvider` affects only custom principals.
