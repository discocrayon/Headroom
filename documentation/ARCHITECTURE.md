...
│   ├── apply.py              # The only place Headroom writes, links, or deletes Terraform
...
sequenceDiagram
    ...
    Tool->>Tool: Compile Terraform Plan
    Tool->>Tool: Apply Terraform Plan
```
...
14. **Generation Is Reconciliation, Not Appending**: A run's output is the complete desired state of the Terraform directory, so a target that drops out of the recommendations loses its file. The boundary is the whole run, not one directory: `compile_terraform_plan` renders org-info, every SCP and RCP file, and the reserved `rcps/grab_org_info.tf` symlink into one validated plan without touching the filesystem, and `apply_terraform_plan` is the only code that writes, links, or deletes. Applying is preflight-then-mutate: it revalidates the plan it was handed rather than trusting it, then a complete read-only pass proves Headroom owns every destination before the first mkdir, and reports every conflict at once rather than the first. Each file is replaced by renaming a fully written sibling over it, never truncated in place: a truncating write that dies leaves a 0-byte file with no marker, which every later run then refuses as somebody else's, wedging the run until a human deletes it. Two output directories that are one directory under two names - a symlink, or a case variant where the filesystem is case-insensitive - are caught by device and inode, because the compiler's path comparison is lexical and cannot see them; where only `mkdir` makes them one, that is caught again the moment the directories exist, still ahead of the first write. A plain typo naming one directory twice is rejected earlier still, when the configuration is read, rather than on the far side of a scan of every account. The reserved symlink's target is computed during apply for the same reason the alias check lives there: a relative link text is relative to where the link really lives, so computing it lexically at compile time produces a link that dangles the moment either directory is spelled as a symlink - and that every later run reads back, agrees with, and leaves. This is not a transaction and must not be described as one - parsing, rendering, validation, and preflight failures mutate nothing, but an OS failure partway through the mutation phase can still leave a partial apply. Ownership is a marker on the file's first line, never a filename pattern or a side manifest: a pattern claims the hand-written `custom_scps.tf` next to ours, and a manifest is separate state that orphans everything it loses and over-deletes whatever it names wrongly. Appending was the original behaviour, and it meant a policy that moved from an OU down to individual accounts kept its OU-wide attachment - still denying the account whose new violation caused the move. Because deleting everything is also how a broken run would present, a run that parses zero result files aborts instead: no evidence is not the same answer as no policies needed
15. **Data Flow**:
