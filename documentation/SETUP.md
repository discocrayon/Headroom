...
Headroom recognizes its own output by the first line of the file:

```
# Code generated by Headroom. DO NOT EDIT.
```

A `.tf` file in `scps_dir` or `rcps_dir` whose first line is anything else is not Headroom's
to replace, so the run aborts before writing, linking, or deleting anything, and lists every
such file at once rather than the first. Move the file aside, rename it, or -- if it is
output from a Headroom old enough to predate the marker -- delete it, and this run will
write it again. Files that are not `.tf`, subdirectories, and symlinks other than the
reserved `rcps/grab_org_info.tf` are never touched and never need moving.
